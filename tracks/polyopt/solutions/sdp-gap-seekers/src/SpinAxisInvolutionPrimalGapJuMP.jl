module SpinAxisInvolutionPrimalGapJuMP
...
export SpinAxisReducedJuMPPrimalModel,
       spin_axis_block_name,
       build_spin_axis_reduced_jump_primal

"""JuMP model after the exact spin-axis involution quotient."""
struct SpinAxisReducedJuMPPrimalModel
    model::JuMP.Model
...
function spin_axis_block_name(block::SpinAxisReducedPSDBlock)
...
function moment_index_map(moments::Vector{MomentKey})
    length(unique(moments)) == length(moments) ||
        throw(ArgumentError("spin-axis moment inventory contains duplicates"))
...
function jump_real_block(
    ...
    for row in 1:dimension, column in row:dimension
        ...
        all(iszero ∘ imag, values(polynomial.terms)) ||
            error("spin-axis real block retained an imaginary coefficient")
...
function add_real_equality!(
    ...
    all(iszero ∘ imag, values(equality.terms)) ||
        error("spin-axis equality $index is not exactly real")
    expression = real(jump_affine_expression(
        equality,
        moment_variables,
        moment_indices,
    ...
    return JuMP.@constraint(
        model,
        expression == 0.0,
        base_name="spin_axis_reduced_equality[$index]",
    )
end

function build_spin_axis_reduced_jump_primal(
    assembly::SpinAxisReducedPrimalAssembly;
    ...
    moment_variables = JuMP.@variable(
        ...
        base_name="spin_axis_invariant_moment",
    )
    ...
    for block in [assembly.positive_blocks; assembly.gap_blocks]
        ...
        push!(
            ...
            JuMP.@constraint(
                ...
                base_name=spin_axis_block_name(block),
            ),
        )
    end

    return SpinAxisReducedJuMPPrimalModel(
        model,
        moment_variables,
        normalization,
        equality_constraints,
