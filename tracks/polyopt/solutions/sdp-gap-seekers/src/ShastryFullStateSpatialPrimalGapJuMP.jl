module ShastryFullStateSpatialPrimalGapJuMP
...
export ShastryFullStateSpatialJuMPPrimalModel,
       shastry_full_state_spatial_block_name,
       build_shastry_full_state_spatial_jump_primal

struct ShastryFullStateSpatialJuMPPrimalModel
    model::JuMP.Model
...
function shastry_full_state_spatial_block_name(
...
function moment_index_map(moments::Vector{MomentKey})
    length(unique(moments)) == length(moments) ||
        throw(ArgumentError("Shastry spatial moment inventory contains duplicates"))
...
function jump_real_block(
    ...
    for row in 1:dimension, column in row:dimension
        ...
        all(iszero ∘ imag, values(polynomial.terms)) ||
            error("Shastry spatial block retained an imaginary coefficient")
...
function add_real_equality!(
    ...
    all(iszero ∘ imag, values(equality.terms)) ||
        error("Shastry spatial equality $index is not exactly real")
    expression = real(jump_affine_expression(
        equality,
        moment_variables,
        moment_indices,
    ...
    return JuMP.@constraint(
        model,
        expression == 0.0,
        base_name="shastry_l1d2_spatial_equality[$index]",
    )
end

function build_shastry_full_state_spatial_jump_primal(
    assembly::ShastryFullStateSpatialReducedPrimalAssembly;
    ...
    for block in [assembly.positive_blocks; assembly.gap_blocks]
        ...
        push!(
            ...
            JuMP.@constraint(
                ...
                base_name=shastry_full_state_spatial_block_name(block),
            ),
        )
    end

    return ShastryFullStateSpatialJuMPPrimalModel(
        model,
        moment_variables,
        normalization,
        equality_constraints,
