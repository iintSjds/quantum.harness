module ContinuousSpinPrimalGapJuMP
...
export ContinuousSpinJuMPPrimalModel,
       continuous_spin_block_name,
       build_continuous_spin_reduced_jump_primal

struct ContinuousSpinJuMPPrimalModel
    model::JuMP.Model
...
function continuous_spin_block_name(block::FullSpinIsotypicPSDBlock)
...
function moment_index_map(moments::Vector{MomentKey})
    length(unique(moments)) == length(moments) ||
        throw(
            ArgumentError(
                "continuous-spin moment inventory contains duplicates",
            ),
        )
...
function jump_real_block(
    ...
    for row in 1:dimension, column in row:dimension
        ...
        all(iszero ∘ imag, values(polynomial.terms)) ||
            error("continuous-spin block retained an imaginary coefficient")
...
function add_real_equality!(
    ...
    all(iszero ∘ imag, values(equality.terms)) ||
        error("continuous-spin equality $index is not exactly real")
    expression = real(jump_affine_expression(
        equality,
        moment_variables,
        moment_indices,
    ...
    return JuMP.@constraint(
        model,
        expression == 0.0,
        base_name="continuous_spin_equality[$index]",
    )
end

function build_continuous_spin_reduced_jump_primal(
    assembly::ContinuousSpinReducedPrimalAssembly;
    ...
    moment_variables = JuMP.@variable(
        ...
        base_name="continuous_spin_invariant_moment",
    )
    ...
    for block in [assembly.positive_blocks; assembly.gap_blocks]
        ...
        push!(
            ...
            JuMP.@constraint(
                ...
                base_name=continuous_spin_block_name(block),
            ),
        )
    end

    return ContinuousSpinJuMPPrimalModel(
        model,
        moment_variables,
        normalization,
        equality_constraints,
