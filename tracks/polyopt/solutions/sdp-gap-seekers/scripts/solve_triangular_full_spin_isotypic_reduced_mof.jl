...
const RESULT_SCHEMA =
    "triangular-heisenberg-full-spin-isotypic-real-solve-result-v1"
...
const EXPECTED_SOURCE_FILES = Set([
    ...
    "build_triangular_full_spin_isotypic_reduced_mof.jl",
...
function progress(message::AbstractString)
    println("[tri-full-spin-isotypic-solve] ", message)
    flush(stdout)
end
...
function write_primal_values(
    ...
    open(temporary, "w") do io
        println(
            io,
            "# schema=triangular-heisenberg-full-spin-isotypic-primal-values-v1",
        )
        println(
            io,
            "# model_mof_sha256=",
    ...
    return Dict(
        "schema_version" =>
            "triangular-heisenberg-full-spin-isotypic-primal-values-v1",
