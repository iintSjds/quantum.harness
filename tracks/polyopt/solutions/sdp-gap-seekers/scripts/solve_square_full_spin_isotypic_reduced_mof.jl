...
const EXPECTED_SOURCE_FILES = Set([
    ...
    "build_square_full_spin_isotypic_reduced_mof.jl",
...
function progress(message::AbstractString)
    println("[square-full-spin-isotypic-solve] ", message)
    flush(stdout)
end

function dynamic_scan_input_enabled()
    value = get(ENV, "SQUARE_SCAN_DYNAMIC_INPUT", "0")
    value == "1" ||
        error("SQUARE_SCAN_DYNAMIC_INPUT must be exactly 1")
    return true
end
