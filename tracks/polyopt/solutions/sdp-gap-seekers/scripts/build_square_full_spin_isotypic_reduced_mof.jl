...
function progress(message::AbstractString)
    println("[square-full-spin-isotypic-mof] ", message)
    flush(stdout)
end
...
function parse_args(arguments::Vector{String})
    ...
    while index <= length(arguments)
        ...
        elseif argument in ("-h", "--help")
            println(
                "usage: build_square_full_spin_isotypic_reduced_mof.jl " *
...
function source_metadata()
    files = [
        ...
        "tracks/polyopt/solutions/sdp-gap-seekers/scripts/build_square_full_spin_isotypic_reduced_mof.jl",
    ]
...
function verify_reloaded_model(
    ...
    report = full_spin_isotypic_reduced_assembly_report(assembly)
    JuMP.num_variables(model) == report.isotypic_moments ||
    ...
    for index in eachindex(assembly.equalities)
        isnothing(JuMP.constraint_by_name(
            model,
            "full_spin_isotypic_equality[$index]",
    ...
    for block in blocks
        name = full_spin_isotypic_block_name(block)
    ...
    return Dict(
        "passed" => true,
        "variable_count" => report.isotypic_moments,
...
function main(arguments::Vector{String}=ARGS)
    ...
    progress("optimizer-free full-spin isotypic JuMP model")
    jump_measurement =
        @timed build_full_spin_isotypic_reduced_jump_primal(isotypic_reduced)
    ...
    progress("reload and verify full-spin isotypic MOF")
    ...
    progress(
        "complete; moments=$(isotypic_report.isotypic_moments), " *
        "psd_blocks=$(length(isotypic_reduced.positive_blocks) + length(isotypic_reduced.gap_blocks)), " *
        "real_psd_entries=$(isotypic_report.real_psd_triangle_entries), " *
