...
function progress(message::AbstractString)
    println("[ss-full-spin-mof] ", message)
    flush(stdout)
end
...
function parse_args(arguments::Vector{String})
    ...
    while index <= length(arguments)
        ...
        elseif argument in ("-h", "--help")
            println(
                "usage: build_shastry_sutherland_full_spin_reduced_mof.jl " *
...
function source_metadata()
    files = [
        ...
        "tracks/polyopt/solutions/sdp-gap-seekers/scripts/build_shastry_sutherland_full_spin_reduced_mof.jl",
    ]
...
function verify_reloaded_model(
    ...
    report = full_spin_reduced_assembly_report(assembly)
    JuMP.num_variables(model) == report.full_spin_moments ||
    ...
    for index in eachindex(assembly.equalities)
        isnothing(JuMP.constraint_by_name(
            model,
            "full_spin_reduced_equality[$index]",
    ...
    for block in blocks
        name = full_spin_block_name(block)
    ...
    return Dict(
        "passed" => true,
        "variable_count" => report.full_spin_moments,
...
function main(arguments::Vector{String}=ARGS)
    ...
    progress("optimizer-free full-spin real-cone JuMP model")
    jump_measurement =
        @timed build_full_spin_reduced_jump_primal(full_spin)
    ...
    progress("reload and verify full-spin MOF")
    ...
    progress(
        "complete; moments=$(full_spin_report.full_spin_moments), " *
        "psd_blocks=$(length(full_spin.source.positive_blocks) + length(full_spin.source.gap_blocks)), " *
