...
const PATCH_LEVEL = 1
const DEGREE_D = 3
const DECISION_GAMMA = BigInt(2) // BigInt(1)

function progress(message::AbstractString)
    println("[square-l1d3-spatial-reflection-mof] ", message)
    flush(stdout)
end
...
function parse_args(arguments::Vector{String})
    ...
    while index <= length(arguments)
        ...
        elseif argument in ("-h", "--help")
            println(
                "usage: build_square_l1d3_spatial_reflection_reduced_mof.jl " *
    ...
    gamma == DECISION_GAMMA ||
        throw(ArgumentError("the decision build is locked to gamma=2"))
    return (
        output=output_path,
...
function source_metadata()
    files = [
        ...
        "tracks/polyopt/solutions/sdp-gap-seekers/src/FullSpinIsotypicReduction.jl",
        "tracks/polyopt/solutions/sdp-gap-seekers/src/FullSpinIsotypicPrimalGapJuMP.jl",
        "tracks/polyopt/solutions/sdp-gap-seekers/src/SpatialReflectionReduction.jl",
        "tracks/polyopt/solutions/sdp-gap-seekers/src/SpatialReflectionPrimalGapJuMP.jl",
...
function verify_reloaded_model(
    ...
    report = spatial_reflection_reduced_assembly_report(assembly)
    ...
    for index in eachindex(assembly.equalities)
        isnothing(JuMP.constraint_by_name(
            model,
            "spatial_reflection_equality[$index]",
        )) && error("MOF lost spatial-reflection equality $index")
    end

    dimensions = Dict{String,Int}()
    blocks = [assembly.positive_blocks; assembly.gap_blocks]
    for block in blocks
        name = spatial_reflection_block_name(block)
    ...
    return Dict(
        "passed" => true,
        "variable_count" => report.spatial_moments,
...
function main(arguments::Vector{String}=ARGS)
    ...
    problem = GapProblem(
        square_patch_geometry(PATCH_LEVEL),
    ...
    progress(
        "source inventory; moments=$(length(primal.moments)), " *
        "positive_side=$(length(primal.positive_basis.entries)), " *
        "gap_side=$(length(primal.gap_basis.entries)), " *
        "equalities=$(length(primal.stationarity_equalities)), " *
    ...
    progress(
        "V4 inventory; moments=$(v4_report.reduced_moments), " *
        "positive_sides=$(join(v4_report.positive_block_dimensions, ',')), " *
        "gap_sides=$(join(v4_report.gap_block_dimensions, ',')), " *
        "peak_rss_kib=$(peak_rss_kib())",
    )
    ...
    progress(
        "conjugation inventory; moments=$(conjugation_report.real_moments), " *
        "real_psd_entries=$(conjugation_report.real_psd_triangle_entries), " *
        "peak_rss_kib=$(peak_rss_kib())",
    )
    ...
    progress(
        "spin-axis inventory; moments=$(spin_axis_report.spin_axis_moments), " *
        "positive_sides=$(join(spin_axis_report.positive_block_dimensions, ',')), " *
        "gap_sides=$(join(spin_axis_report.gap_block_dimensions, ',')), " *
        "peak_rss_kib=$(peak_rss_kib())",
    )
    ...
    progress(
        "full-spin inventory; moments=$(full_spin_report.full_spin_moments), " *
        "real_psd_entries=$(full_spin_report.real_psd_triangle_entries), " *
        "peak_rss_kib=$(peak_rss_kib())",
    )
    ...
    progress(
        "cone inventory; moments=$(cone_report.cone_reduced_moments), " *
        "positive_sides=$(join(cone_report.positive_block_dimensions, ',')), " *
        "gap_sides=$(join(cone_report.gap_block_dimensions, ',')), " *
        "peak_rss_kib=$(peak_rss_kib())",
    )

    progress("exhaustive full-spin trivial-isotypic truth gate")
    isotypic_truth_measurement =
        @timed full_spin_trivial_isotypic_truth(cone_reduced)
    isotypic_truth = isotypic_truth_measurement.value
    isotypic_truth.exact ||
        error("full-spin trivial-isotypic truth gate failed")

    progress("exact full-spin trivial-isotypic reduction")
    isotypic_measurement = @timed assemble_full_spin_isotypic_reduced_primal(
        cone_reduced;
        verify_truth=false,
    )
    isotypic_reduced = isotypic_measurement.value
    isotypic_report =
        full_spin_isotypic_reduced_assembly_report(isotypic_reduced)
    progress(
        "isotypic inventory; moments=$(isotypic_report.isotypic_moments), " *
        "positive_sides=$(join(isotypic_report.positive_block_dimensions, ',')), " *
        "gap_sides=$(join(isotypic_report.gap_block_dimensions, ',')), " *
        "peak_rss_kib=$(peak_rss_kib())",
    )

    progress("exhaustive anti-diagonal spatial-reflection truth gate")
    spatial_truth_measurement =
        @timed spatial_reflection_reduction_truth(isotypic_reduced)
    spatial_truth = spatial_truth_measurement.value
    spatial_truth.exact ||
        error("spatial-reflection truth gate failed")

    progress("exact anti-diagonal spatial-reflection reduction")
    spatial_measurement =
        @timed assemble_spatial_reflection_reduced_primal(
            isotypic_reduced;
            verify_truth=false,
        )
    spatial_reduced = spatial_measurement.value
    spatial_report =
        spatial_reflection_reduced_assembly_report(spatial_reduced)
    progress(
        "spatial inventory; moments=$(spatial_report.spatial_moments), " *
        "positive_sides=$(join(spatial_report.positive_block_dimensions, ',')), " *
        "gap_sides=$(join(spatial_report.gap_block_dimensions, ',')), " *
        "real_psd_entries=$(spatial_report.real_psd_triangle_entries), " *
        "max_side=$(spatial_report.maximum_psd_side_dimension), " *
        "peak_rss_kib=$(peak_rss_kib())",
    )

    progress("optimizer-free spatial-reflection JuMP model")
    jump_measurement =
        @timed build_spatial_reflection_reduced_jump_primal(spatial_reduced)
    ...
    progress("reload and verify spatial-reflection MOF")
    ...
    runmeta = Dict(
        ...
        "setup" => Dict(
            ...
            "g_j2_over_j1" => rational_metadata(J2_OVER_J1),
        ...
        "exact_full_spin_isotypic_reduction" => Dict(
            "schema" => isotypic_reduced.schema,
            "assembly_sha256" => isotypic_reduced.assembly_sha256,
            "coefficient_map_sha256" =>
                isotypic_reduced.coefficient_map_sha256,
            "moment_count" => isotypic_report.isotypic_moments,
            "eliminated_unused_moment_count" =>
                isotypic_report.eliminated_unused_moments,
            "positive_block_dimensions" =>
                isotypic_report.positive_block_dimensions,
            "gap_block_dimensions" =>
                isotypic_report.gap_block_dimensions,
            "equality_count" => isotypic_report.equality_count,
            "real_psd_triangle_entries" =>
                isotypic_report.real_psd_triangle_entries,
            "maximum_psd_side_dimension" =>
                isotypic_report.maximum_psd_side_dimension,
            "source_block_dimensions" =>
                isotypic_truth.source_dimensions,
            "trivial_block_dimensions" =>
                isotypic_truth.trivial_dimensions,
            "standard_block_dimensions" =>
                isotypic_truth.standard_dimensions,
            "singleton_orbit_count" =>
                isotypic_truth.singleton_orbit_count,
            "triple_orbit_count" =>
                isotypic_truth.triple_orbit_count,
            "row_actions_unsigned" =>
                isotypic_truth.row_actions_unsigned,
            "conjugation_rows_even" =>
                isotypic_truth.conjugation_rows_even,
            "involution_exact" =>
                isotypic_truth.involution_exact,
            "cross_blocks_zero" =>
                isotypic_truth.cross_blocks_zero,
            "cross_entry_count" =>
                isotypic_truth.cross_entry_count,
            "standard_blocks_proportional" =>
                isotypic_truth.standard_blocks_proportional,
            "standard_proportionality_factor" =>
                isotypic_truth.standard_proportionality_factor,
            "standard_relation_entry_count" =>
                isotypic_truth.standard_relation_entry_count,
            "bases_invertible" =>
                isotypic_truth.bases_invertible,
            "basis_dimensions" =>
                isotypic_truth.basis_dimensions,
            "truth_checks_exhaustive" => isotypic_truth.exact,
        ),
        "exact_spatial_reflection_reduction" => Dict(
            "schema" => spatial_reduced.schema,
            "assembly_sha256" => spatial_reduced.assembly_sha256,
            "coefficient_map_sha256" =>
                spatial_reduced.coefficient_map_sha256,
            "source_moment_count" =>
                spatial_report.source_isotypic_moments,
            "moment_count" => spatial_report.spatial_moments,
            "eliminated_spatial_moment_count" =>
                spatial_report.eliminated_spatial_moments,
            "raw_nonrepresentative_count" =>
                spatial_truth.raw_nonrepresentative_count,
            "positive_block_dimensions" =>
                spatial_report.positive_block_dimensions,
            "gap_block_dimensions" =>
                spatial_report.gap_block_dimensions,
            "equality_count" => spatial_report.equality_count,
            "real_psd_triangle_entries" =>
                spatial_report.real_psd_triangle_entries,
            "maximum_psd_side_dimension" =>
                spatial_report.maximum_psd_side_dimension,
            "site_map" => spatial_truth.site_map,
            "site_map_involutive" =>
                spatial_truth.site_map_involutive,
            "hamiltonian_invariant" =>
                spatial_truth.hamiltonian_invariant,
            "coefficient_covariant" =>
                spatial_truth.coefficient_covariant,
            "coefficient_count" =>
                spatial_truth.coefficient_count,
            "equality_space_invariant" =>
                spatial_truth.equality_space_invariant,
            "stable_cross_blocks_zero" =>
                spatial_truth.stable_cross_blocks_zero,
            "stable_cross_entry_count" =>
                spatial_truth.stable_cross_entry_count,
            "stable_bases_invertible" =>
                spatial_truth.stable_bases_invertible,
            "stable_basis_dimensions" =>
                spatial_truth.stable_basis_dimensions,
            "truth_checks_exhaustive" => spatial_truth.exact,
        ),
        "timing" => Dict(
            ...
            "spatial_reflection_truth" =>
                timed_metadata(spatial_truth_measurement),
            "spatial_reflection_reduction" =>
                timed_metadata(spatial_measurement),
    ...
    progress(
        "complete; moments=$(spatial_report.spatial_moments), " *
        "psd_blocks=$(length(spatial_reduced.positive_blocks) + length(spatial_reduced.gap_blocks)), " *
        "real_psd_entries=$(spatial_report.real_psd_triangle_entries), " *
